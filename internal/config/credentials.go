...
func ReadClientCredentialsFor(client string) (ClientCredentials, error) {
	...
	if b, err = os.ReadFile(path); err != nil { //nolint:gosec // user-provided path
		if os.IsNotExist(err) {
			// If the default client credentials file doesn't exist, try embedded credentials
			if client == DefaultClientName && HasEmbeddedCredentials() {
				return GetEmbeddedCredentials()
			}
...
func ClientCredentialsExists(client string) (bool, error) {
	...
	if _, err := os.Stat(path); err != nil {
		if os.IsNotExist(err) {
			// If the default client credentials file doesn't exist, check for embedded credentials
			if client == DefaultClientName && HasEmbeddedCredentials() {
				return true, nil
			}
			return false, nil
